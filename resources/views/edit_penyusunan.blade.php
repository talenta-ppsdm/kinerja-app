...
<div class="content" style="margin-left: 105px; padding: 8px;">
    <div class="org-container">
        <h2 class="text-light mb-3">Edit Data Penyusunan</h2>

        <!-- Filter Data -->
        <div class="block block-rounded">
            <div class="block-content">
                <form action="{{ route('penyusunan.update', $skpPenyusunan->id) }}" method="POST">
                    @csrf
                    @method('PUT')

                    <div class="form-group">
                        <label for="nama" class="text-dark">Nama</label>
                        <input type="text" class="form-control" id="nama" name="nama" value="{{ $skpPenyusunan->masterSkp->nama }}" readonly>
                    </div>
                    <div class="form-group">
                        <label for="nip" class="text-dark">NIP</label>
                        <input type="text" class="form-control" id="nip" name="nip" value="{{ $skpPenyusunan->masterSkp->nip }}" readonly>
                    </div>
                    <div class="form-group">
                        ...
                        <select class="form-control" id="select-unor" name="unit_organisasi" onchange="updateModalUnker()">
                            ...
                            @foreach($listUnor as $unor)
                                <option value="{{ $unor->value }}" 
                                    {{ $skpPenyusunan->masterSkp->unit_organisasi == $unor->value ? 'selected' : '' }}>
                                    {{ $unor->value }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control" id="select-unker" name="unit_kerja">
                            ...
                            @if($skpPenyusunan->masterSkp->unit_organisasi)
                                @php
                                    $selectedUnor = \App\Enums\UnitOrganisasiEnum::tryFrom($skpPenyusunan->masterSkp->unit_organisasi);
                                    $unitKerjaOptions = $selectedUnor ? $selectedUnor->getUnitKerja() : [];
                                @endphp
                                @foreach($unitKerjaOptions as $unker)
                                    <option value="{{ $unker }}" 
                                        {{ $skpPenyusunan->masterSkp->unit_kerja == $unker ? 'selected' : '' }}>
                                        {{ $unker }}
                                    </option>
                                @endforeach
                            @endif
